/**
 * Direct execution fallback for local dev when Docker is not available.
 * Runs code via child_process without sandboxing — DEV ONLY.
 */
import { spawn } from "child_process";
import { SupportedLanguage, EXECUTION_TIMEOUT_MS, MAX_OUTPUT_BYTES } from "./languages.js";
import type { StreamCallback, DoneCallback, ErrorCallback } from "./docker-runner.js";
...
function spawnDirect(
  ...
  child.stderr.on("data", (data: Buffer) => {
    const chunk = data.toString("utf8");
    stderrBuffer += chunk;
    if (!truncated) onChunk(chunk, "stderr");
  });
